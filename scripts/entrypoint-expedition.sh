#!/usr/bin/env bash
# =============================================================================
# OntDekker — expedition-service container entrypoint
#
# Startup sequence:
...
#                   postgresql+asyncpg://postgres:postgres@postgres:5432/trip_db
# =============================================================================
set -e
...
echo "[expedition-service] Waiting for PostgreSQL at ${DB_HOST}:${DB_PORT}..."
...
until pg_isready -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -q; do
    ...
    if [ "$attempt" -ge "$MAX_RETRIES" ]; then
        echo "[expedition-service] ERROR: PostgreSQL not ready after ${MAX_RETRIES} attempts. Aborting."
        exit 1
    fi
    echo "[expedition-service] PostgreSQL not ready yet (attempt ${attempt}/${MAX_RETRIES}). Retrying in ${RETRY_INTERVAL}s..."
    sleep "$RETRY_INTERVAL"
done

echo "[expedition-service] PostgreSQL is ready."

# ---------------------------------------------------------------------------
# 2. Run Alembic migrations.
#    alembic upgrade head is idempotent — it is a no-op if already at head.
# ---------------------------------------------------------------------------
echo "[expedition-service] Running Alembic migrations..."
alembic upgrade head
echo "[expedition-service] Migrations complete."
...
# ---------------------------------------------------------------------------
echo "[expedition-service] Starting uvicorn..."
